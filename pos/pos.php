...
let appliedPayments = []; // Stores {type, amount, label}
let customerId = 0; // Current customer ID
...
function prepareAndOpenPaymentModal() {
    ...
    const credit = parseFloat(document.getElementById('customer_due_balance').value) || 0;
    ...
    if (openingBal > 0) {
        pmOpeningBal.style.display = 'flex';
        pmOpeningBalVal.textContent = openingBal.toFixed(2);
    } else {
        pmOpeningBal.style.display = 'none';
        pmOpeningBal.classList.remove('selected');
        if (selectedPaymentMethod === 'opening_balance') selectedPaymentMethod = null;
    }

    // Toggle Gift Card method
    const pmGiftcard = document.getElementById('pm-giftcard');
    const pmGiftcardBal = document.getElementById('pm-giftcard-balance');
    if (giftcardBal > 0) {
        pmGiftcard.style.display = 'flex';
        pmGiftcardBal.textContent = giftcardBal.toFixed(2);
    } else {
        pmGiftcard.style.display = 'none';
        pmGiftcard.classList.remove('selected');
    ...
    // Toggle Credit method
    const pmCredit = document.getElementById('pm-credit');
    const pmCreditBal = document.getElementById('pm-credit-balance');
    if (credit > 0 || customerId === 0) { // Credit (Due) always available for registered customers, or not?
        // Actually, 'Credit' in the sidebar here refers to paying LATER (creating due)
        // User wants Opening Balance as "Opening Balance".
        pmCredit.style.display = 'flex';
        pmCreditBal.textContent = credit.toFixed(2);
    } else {
        pmCredit.style.display = 'none';
        pmCredit.classList.remove('selected');
        // If it was selected, deselect it
    ...
    updatePaymentSummary(); // Ensure totals are up to date
...
function completeSale() {
    ...
    let amountVal = amountReceivedInput ? parseFloat(amountReceivedInput.value) : 0;
    
    // Fallback: If amountVal is 0, but "Full Payment" is selected or we remain on the "Pay Amount" tab,
    // we should assume the user wants to pay the full amount.
    const isFullPaymentTab = document.getElementById('tab-pay-amount').style.display !== 'none';
    const activePaymentTypeBtn = document.querySelector('.payment-type-btn.active');
    const isFullPaymentType = activePaymentTypeBtn && activePaymentTypeBtn.dataset.type === 'full';

    const grandTotal = parseFloat(document.getElementById('payment-payable').textContent) || 0;
    const totalPayableWithDue = parseFloat(document.getElementById('payment-total-payable').textContent) || 0;
    ...
    formData.append('amount_received', amountVal);
    ...
    .then(data => {
        if(data.success) {
            ...
            document.getElementById('inv-paid').textContent = document.getElementById('payment-paid').textContent;
            
            const grandTotalSale = parseFloat(document.getElementById('payment-payable').textContent) || 0;
            const paidAmountTotal = parseFloat(document.getElementById('payment-paid').textContent) || 0;
            const previousDueTotal = parseFloat(document.getElementById('payment-previous-due')?.textContent) || 0;
            const totalPayableAll = grandTotalSale + previousDueTotal;

            let currentSaleUnpaid = Math.max(0, grandTotalSale - paidAmountTotal);
            let remainingPrevDue = Math.max(0, totalPayableAll - paidAmountTotal - currentSaleUnpaid);
            const totalRemainingDue = currentSaleUnpaid + remainingPrevDue;
            
            document.getElementById('inv-due').textContent = currentSaleUnpaid.toFixed(2);
            document.getElementById('inv-prev-due').textContent = remainingPrevDue.toFixed(2);
            document.getElementById('inv-total-due').textContent = totalRemainingDue.toFixed(2);
            
            // Amount in words
            const grandTotal = parseFloat(document.getElementById('payment-payable').textContent) || 0;
            document.getElementById('inv-in-words').textContent = numberToWords(grandTotal);
            
            // Payment Method
            let pmName = 'Cash';
            if (selectedPaymentMethod === 'giftcard') pmName = 'Gift Card';
            else if (selectedPaymentMethod === 'credit') pmName = 'Credit';
            else {
                const pmEl = document.querySelector(`.sidebar-payment-method[data-id="${selectedPaymentMethod}"] span`);
                if(pmEl) pmName = pmEl.textContent;
            }
            document.getElementById('inv-payment-method').textContent = pmName;
            document.getElementById('inv-payment-amount').textContent = document.getElementById('payment-paid').textContent;
            ...
            // Check if it's a Walking Customer error
            if(data.is_walking_customer_error) {
                closeModal('paymentModal');
                Swal.fire({
                    icon: 'warning',
